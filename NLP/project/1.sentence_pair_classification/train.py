...
def train(args, train_dataset, dev_dataset, model, tokenizer):
    ...
    t_total = len(train_dataloader) * args.num_train_epochs  # 总共的训练步数
    ...
    args.warmup_steps = int(t_total * args.warmup_proportion)  # 计算预热步数
    optimizer = AdamW(
        optimizer_grouped_parameters,
        ...
        betas=(
            args.adam_beta1,
            args.adam_beta2,
        ),  # 这两个参数用于计算梯度和梯度的平方的指数移动平均值。
        eps=args.adam_epsilon,  # 设置了一个小的常数，用于防止除0错误。
    )
...
if __name__ == "__main__":
    ...
    model = (
        ...
        .from_pretrained(args.model_checkpoint, config=config, args=args)
        .to(args.device)
    )
    # Training
    if args.do_train:
    ...
    if args.do_predict:
        ...
        for save_weight in save_weights:
            ...
            for s_idx in tqdm(range(len(test_dataset))):
                ...
                pred, prob = predict(
                    args, sample["sentence1"], sample["sentence2"], model, tokenizer
                )
                results.append(
                    {
            ...
            with open(
                os.path.join(args.output_dir, save_weight + "_test_pred_simi.json"),
                "wt",
                encoding="utf-8",
            ) as f:
